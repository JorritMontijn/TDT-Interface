...
if ~isfield(ops,'root') || isempty(ops.root)
ops.root                = 'D:\Data\Raw\ePhys\KiloSortBinaries'; % 'openEphys' only: where raw files are
end
if ~isfield(ops,'rec') || isempty(ops.rec)
ops.rec                 = 'Roku_20180514_B3'; %which recording to process
end
ops.fbinary             = [ops.root filesep ops.rec filesep ops.rec '_all.bin']; % will be created for 'openEphys'		
ops.fproc               = [ops.root filesep ops.rec filesep ops.rec '_res.dat']; % residual from RAM of preprocessed data
	
% define the channel map as a filename (string) or simply an array		
ops.chanMap             = [ops.root filesep 'chanMap.mat']'; % make this file using createChannelMapFile.m		
